...
app.post('/webhook/kick', async (req, res) => {
  ...
  try {
    ...
    switch (command) {
      ...
      case '!kd': {
        if (!checkCooldown(sender, 'kd', 10)) return;
        try {
          const TARKOV_AID = 12559296;
          
          // Regular (PVP) profili çek
          const regRes = await fetch(`https://players.tarkov.dev/profile/${TARKOV_AID}.json`, {
            headers: { 'User-Agent': 'TarkovBot/1.0', 'Accept': 'application/json' }
          });
          
          let pvpLine = '❌ PVP verisi alınamadı';
          let seasonLine = '❌ Sezon verisi alınamadı';
          ...
          const seasonRes = await fetch(`https://players.tarkov.dev/profile/${TARKOV_AID}.json`, {
            headers: { 'User-Agent': 'TarkovBot/1.0', 'Accept': 'application/json' }
          });
          
          // Season verisi regular JSON'dan ayrı endpoint'te olabilir, site /pvp-season/ kullanıyor
          // Doğrudan pvp-season endpoint'ini dene
          try {
            const sRes = await fetch(`https://players.tarkov.dev/pvp-season/${TARKOV_AID}.json`, {
              headers: { 'User-Agent': 'TarkovBot/1.0', 'Accept': 'application/json' }
            });
            if (sRes.ok) {
          ...
          } catch(e) { /* season endpoint yoksa skip */ }
          
          await sendKickMessage(`${pvpLine} | ${seasonLine}`, channelId);
        } catch (err) {
          console.error('KD hatası:', err.message);
          await sendKickMessage('❌ Tarkov istatistikleri alınamadı.', channelId);
        }
        break;
      }
